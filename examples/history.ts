...
const resp1 = await adapter.fetchMessages(
  adapter.encodeThreadId({ chatType: 'private', peerId: '834458085' }),
  { limit: 5 }
);
...
if (resp1.nextCursor) {
  const resp2 = await adapter.fetchMessages(process.env.NAPCAT_THREAD_ID!, {
    limit: 5,
